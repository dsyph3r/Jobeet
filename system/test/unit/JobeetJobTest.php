...
$job = Doctrine_Core::getTable('JobeetJob')->createQuery()->fetchOne();
$t->is($job->getPositionSlug(), Jobeet::slugify($job->getPosition()), '->getPositionSlug() return the slug for the company');

$t->comment('->getLocationSlug()');
$job = Doctrine_Core::getTable('JobeetJob')->createQuery()->fetchOne();
$t->is($job->getLocationSlug(), Jobeet::slugify($job->getLocation()), '->getLocationSlug() return the slug for the company');
